...
public class Pager implements Serializable {
	...
	private int recordTotal = 0;	// 总条数
	private int previousPage;		// 前一页
	private int nextPage;			// 下一页
	private int firstPage = 1;		// 第一页
	private int lastPage;			// 最后一页
	private int startPage;			// 页号式导航, 起始页号
	private int endPage;			// 页号式导航, 结束页号
	...
	public void otherAttr() {
		 ...
		 * 同时需要判断是不是第一页，第一页不需要向前显示翻页数字。 this.currentPage - numCount / 2 只要本页不大于6
		 * 它的数字翻页始终从1开始，7就从2开始 8就从3开始 以此类推
		 */
